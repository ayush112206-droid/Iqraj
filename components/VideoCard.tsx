...
import { Play, Copy, ExternalLink, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function VideoCard({ video, index }: VideoCardProps) {
  ...
  const title = video.name || video.title || `Lecture ${index + 1}`;
  ...
  return (
    ...
    >
      ...
      >
        <div className="w-10 h-10 rounded-lg bg-video-accent/10 flex items-center justify-center flex-shrink-0">
          <Play className="w-5 h-5 text-video-accent fill-video-accent" />
        </div>
